...
FallingThingsStereo(
    root: str | Path,
    variant: str = "single",  # "single" | "mixed" | "fat"
    transforms: Callable | None = None,
)
```
...
|---|---|---|---|
| `Kitti2012Stereo` | `train` / `test` | No | ❌ Manual |
| `Kitti2015Stereo` | `train` / `test` | No | ❌ Manual |
| `CarlaStereo` | training only | No | ❌ Manual |
| `Middlebury2014Stereo` | `train` / `additional` / `test` | No | ✅ |
| `CREStereo` | training only | No | ❌ Manual |
| `FallingThingsStereo` | `single` / `mixed` / `fat` variants | No | ❌ Manual |
| `SceneFlowStereo` | training only | No | ❌ Manual |
| `SintelStereo` | training only | No | ❌ Manual |
| `InStereo2k` | `train` / `test` | No | ❌ Manual |
| `ETH3DStereo` | `train` / `test` | No | ❌ Manual |
